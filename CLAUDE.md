...
> 定位一句话：输入公历/农历生日 → 按《宿曜经》本命宿法（农历生日）算本命宿 → 展示形象、星图与七维解读；带邮箱登录。
...
2. **不臆造**：
   - 测算走**可查权威方法**：本命宿依《宿曜经》原文算法（见 §3），历法换算走 `lunar-javascript`；
     不臆造、不写死未经校验的表，上线前用 ≥3 个已知本命宿样本**交叉校验**。
     〔2026-07-19 更正：原文为「不手写值日表、一律走 lunar-javascript」——值日宿≠本命宿，已改用宿曜经本命宿；
     铁律措辞变更**待 Rick review 本 PR 时明确确认**（改铁律须你点头）。〕
...
| 历法/测算 | **`lunar-javascript`（6tail, MIT）** 做公历↔农历换算 + 时辰地支；**本命宿依《宿曜经》算法**（见 §3，非库的值日宿）|
...
## 3. 本命宿测算（核心引擎 · 《宿曜经》本命宿）

> ⚠️ 值日宿 ≠ 本命宿。老黄历「值日星宿」逐日轮值、逐年变（= `lunar.getXiu()`）；本项目要的是
> **《宿曜经》按农历生日定的「本命宿」**（与年份无关）。详见 PRD §7 更正框。

- **本命宿算法（《宿曜经》原文）**：`命宿 = 从「生月望宿」起、按 27 宿序（娄首去牛）顺数「农历日+13」位`。
  - 望宿表（每月十五之宿）：正月翼·二月角·三月氐·四月心·五月箕·六月女·七月室·八月娄·九月昴·十月觜·冬月鬼·腊月星。
  - 27 宿序：娄胃昴毕觜参井鬼柳星张翼轸角亢氐房心尾箕斗女虚危室壁奎（去「牛」，故本命宿恒不含牛）。
  - 七政+动物由固定「禽星表」给出（如 轸→水+蚓=轸水蚓），**不用** `getZheng()/getAnimal()`（那是值日宿的）。
- **`lunar-javascript` 只做历法换算 + 时辰**：`Solar/Lunar.fromYmd(...)` 取农历月/日（`getMonth()/getDay()`）、
  时辰地支（`getTimeZhi()`）；不手写农历换算表。
- 封装成**可替换模块** `computeBenmingXiu(input)`，页面层不直接调库（经 `/api/compute`）。
- 本命宿按「农历月+日」定；**时辰不改变本命宿**，只驱动「时辰辅助」板块。时辰选「不确定」→ 隐藏该板块，照常出本命宿。
- 交叉校验：**已用 12 样本对权威源核对通过**（PRD §13）；换算/算法有变须重跑。
